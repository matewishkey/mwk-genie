...
| 2 | **Prefix, not namespace.** `/mwk-save`, `/mwk-magic`, `/mwk-learning`, `/mwk-bug`, `/mwk-new` | User-level skills share one flat namespace, so the prefix goes in the directory name. Same readability, no plugin needed. **⤳ Renamed 2026-09-13: `mwk-magic` → `mwk-review`, `mwk-learning` → `mwk-learn`.** The prefix decision stands; only the two names moved. Both were asked for by their function — a beginner reading a menu needs the verb, and "magic" does not say "tell me if I am overcomplicating this". |
